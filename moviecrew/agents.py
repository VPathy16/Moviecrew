...
class CinematographerAgent(Agent):
    ...
    system_prompt = (
        ...
        "duration_s must be between 5 and 15 seconds — a hard production limit: shots under "
        "5s are too brief for the renderer to produce cleanly, and shots over 15s drag pacing "
        "even inside a long scene. Vary durations within that range to match what each beat "
        "actually needs — a simple reaction might be 5-6s, a shot carrying several linked "
        "beats might run 10-15s. Do not default every shot to the same duration; a scene where "
        "every shot is identically 5s reads as monotonous, mechanical cutting, not real "
        "coverage. If a beat feels shorter than 5s on its own, it is not its own shot — fold "
        "it into an adjacent shot's beats instead. "

...
*Duration.* The clamp comes from `capabilities.clamp_duration`, the client's
own answer, not from a constant written here. A backend that grows from 10s
to 30s clips needs no change in this file.
"""
...
class GenerativeVideoBackend(VideoBackend[ShotSpec]):
    ...
    def adapt(
        ...
        # A cap of 0 means this model takes no image references at all, so
        # dropping them is the correct reading, not a bug.
        spec.reference_images = spec.reference_images[
            : capabilities.max_image_references
        ]
        return spec
    ...
    def render(
        ...
        *,
        ...
        try:
            ...
            if job.video_url:
                self.produced_url_by_shot_id[spec.shot_id] = job.video_url

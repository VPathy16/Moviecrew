...
(() => {
 ...
 let selected=0,playing=false,insertAt=0,pendingInsert=null,enhanceId=null,undo=[],dialogVersion=0;
 ...
 function openClipMenu(index,x,y,trigger){
  ...
  action('Split at playhead',()=>{const t=player.currentTime;if(t<=c.start+.05||t>=c.end-.05)return notify('Move the playhead inside this clip to split it');remember();cut.clips.splice(index,1,{...c,end:t},{...c,start:t});changed();renderCut()});
 ...
 const oldLoad=loadFinalEdit;loadFinalEdit=async function(){undo=[];selected=0;userZoomed=false;await oldLoad()};
